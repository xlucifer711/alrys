...
IQTHONPC = gvarstatus("ALIVE_PIC") or "https://telegra.ph/file/c0e6d1301e01d34cdc558.jpg"
...
IQPIC = gvarstatus("ALIVE_PIC") or "https://telegra.ph/file/c0e6d1301e01d34cdc558.jpg"
...
async def iq(iqthonevent):
    ...
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "𝗐𝖾𝗅𝖼𝗈𝗆𝖾 Mrs. Thon 𓃠"
    IQTHON_IMG = gvarstatus("ALIVE_PIC") or "https://telegra.ph/file/c0e6d1301e01d34cdc558.jpg"
...
async def _(event):
    ...
    OUTPUT = (f"🝳︙  وحـدة المعلومات الخاصه بتنصيبك مع جميع الفارات  لتنصيب سورس سيدا ثون @UUUO1944 :**\n\n{o}")
...
async def _(event):
    ...
    await asyncio.sleep(1)
    await edit_or_reply(event , """🝳︙ جـاري تحديث سيدا ثون (7.7)
    ...
    if vname in vlist:
        ...
        if cmd == "اضف":
            if not vinfo and vname == "ALIVE_TEMPLATE":
                return await edit_delete(event, f"**🝳︙  📑 يرجى متابع قناه الفارات تجدها هنا : @UUUO1944")
...
———————×——————— 
الأمر ⦙ ( .المده )
الشرح  : يضهر لك مدة تشغيل بوت سيدا ثون لديك 
———————×———————
الأمر ⦙ ( .تحميل ملف + الرد ع الملف )
الشرح : يحمل ملفات سيدا ثون 
———————×———————
الأمر ⦙ ( .مسح ملف + الرد ع الملف )
الشرح :  يمسح الملف الي حملته  
———————×———————
الأمر ⦙ ( .تحديث )
الشرح :  امر لأعاده التشغيل وتحديث ملفات السورس وتسريع السيدا ثون 
———————×———————
الأمر ⦙ ( .اطفاء مؤقت + عدد الثواني )
الشرح : يقوم بأطفاء السيدا ثون بعدد الثواني الي ضفتها  عندما تخلص الثواني سيتم اعاده تشغيل السيدا ثون 
———————×———————
الأمر ⦙ ( .الاوامر ) 
...
———————×———————
الأمر ⦙ ( .استخدامي )
الشرح :   يضهر لك كمية استخدامك لسيدا ثون
———————×———————
الأمر ⦙ ( .تاريخ التنصيب )
