...
'''
mm = '''
قم بلأنضمام الى قناة سورس كرستين @c_r_source 
'''
...
keyboard = [
  ...
  [
    Button.url("قناة السورس", "https://t.me/c_r_source")
    ]
]
...
@tgbot.on(events.NewMessage(pattern="/raz", func = lambda x: x.is_private))
async def start(event):
  ...
  async with bot.conversation(event.chat_id) as x:
    keyboard = [
      ...
      [
        Button.url("المطور", "https://t.me/iiqllll")
        ]
    ]
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply("لقد تم انهاء جميع الجلسات شكراً لأستخدامك الشيطان.", buttons=keyboard)
...
async def start(event):
    keyboard = [
      ...
      [
        Button.url("القناة", "https://t.me/c_r_source")
        ]
    ]
